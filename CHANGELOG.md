...
- Spell/UI discovery no longer fails solely because exact mangled `eqgame.exe` exports are absent; it now accepts validated cleanroom wrapper evidence for known ROF2 targets, preserves the existing fail-closed fingerprint gate, and reports per-target `evidence_source` plus machine-readable `failure_reason`.
...
- Cross-platform unit coverage for spell/UI discovery decision policy, including wrapper-backed validation without runtime exports, missing-cleanroom-target denial, fingerprint mismatch denial, diagnostic-string gating, and independent target outcomes.
