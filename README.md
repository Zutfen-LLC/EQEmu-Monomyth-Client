...
- Keeps full packet metadata tracing disabled by default unless the local developer explicitly sets `MONOMYTH_ENABLE_FULL_PACKET_TRACE=1`; when packet observation is active, this temporarily widens `PacketObserver` metadata logging from sampled to every observed send/receive line for that short session.
...
- When packet hooks are active, installs a byte-validated multiclass guild-trainer gate on the real ROF2 trainer validator at RVA `0x001af304`; it clears the native trainer-class rejection only when the latest authoritative `statClassesBitmask` contains the clicked trainer's playable class.
...
- Trace-only `OP_MemorizeSpell` send observation is disabled by default and additionally requires `MONOMYTH_ENABLE_MEMORIZE_SEND_TRACE=1` plus validated `MemorizeSendPacketWrapper`, `StartSpellMemorizationPath`, `MemSpellCommitPath`, and post-`CanStartMemming` follow-up targets where available. It logs send attempts and spellbook-state transitions only, never mutates packet bytes, and never changes send ordering or return flow.
- Full packet metadata tracing is disabled by default and additionally requires `MONOMYTH_ENABLE_FULL_PACKET_TRACE=1`. It does not add payload decoding or new hooks; it only changes `PacketObserver` metadata logging from sampled to every observed packet for a short developer-controlled session.
...
- The `CapabilityManifest` line includes `full_packet_trace_dev_opt_in`, `full_packet_trace_allowed`, and `full_packet_trace_reason` so sampled-vs-full packet capture is explicit in startup logs
...
- When `MONOMYTH_ENABLE_FULL_PACKET_TRACE=1` is also present and packet observation is active, `PacketObserverRecv` and `PacketObserverSend` switch from sampled logging to `*_log_policy=all_packets` for that run
...
- When the trainer seam installs, `hook_manager` logs one `guild trainer hook installed` line with the validated validator/gate RVAs and later emits `MulticlassTrainerHook` only when an assigned-class trainer override is actually applied
...
This slice does not rewrite the packet buffer, does not alter the total length, does not alter the call ordering, and still calls the original client wrapper unchanged exactly once. Existing `SpellUsabilityTrace target=CanStartMemming ... memorize_send_correlation=<n> send_observation=pending` lines can be paired with `PacketObserverSend ... memorize_send_correlation=<n>` to show that the wrapper was reached. If the packet pointer is null, too short to contain an opcode, or faults during the bounded opcode read, the log stays bounded with `decode_status=not_decoded` plus a concrete `not_decoded_reason=...` and the companion trace emits `SpellUsabilityTrace target=MemorizeSend status=not_decoded ...`. If `CanStartMemming` succeeds and the next observed event is another `CanStartMemming` or shutdown with no matching send observation, the DLL emits `SpellUsabilityTrace target=MemorizeSend status=not_observed ...` so the absence is grep-friendly. Non-matching correlated sends now additionally log a bounded `packet_prefix_hex`, `caller_site_label` / `caller_site_validation` when the immediate wrapper caller matches one of the currently pinned downstream branches at return RVAs `0x13e1cd` or `0x18caba`, and up to two resolved local relative-call targets from that caller window as `caller_call_1_*` / `caller_call_2_*`. When a resolved target is only a hotpatch jump thunk, the same log line also includes `caller_call_*_terminal_target`, `caller_call_*_terminal_target_rva`, and `caller_call_*_terminal_hop_count`. The same wrapper trace now also opens a bounded spellbook-scribe correlation when it observes `OP_DeleteSpell` (`0x3358`) and emits `SpellUsabilityTrace target=SpellbookScribeSend ...` receipts across the subsequent `OP_DeleteSpell -> OP_MemorizeSpell` send sequence, including the same caller-byte and resolved-call metadata so spellbook click-to-scribe can be traced without relying on inventory right-click. The same gate now also installs a trace-only hook on the validated spellbook mem-start helper `StartSpellMemorizationPath` at `0x662290` / RVA `0x262290`, plus the validated spellbook-side `MemSpellCommitPath` at `0x75e620` / RVA `0x35e620` and the validated real follow-up gate body at `0x8dc0f5` / RVA `0x4dc0f5`. `StartSpellMemorizationPath` logs the spellbook window's copied `before_*` and `after_*` values for offsets `0x234/0x238/0x23c/0x240/0x244`, so later runs can prove whether pending mem state is initialized before commit. `MemSpellCommitPath` contains the known `OP_MemorizeSpell` packet construction and wrapper call, so a correlated `SpellUsabilityTrace target=MemSpellCommitPath ...` line proves the direct commit path was actually reached. Its optional spellbook-state fields and live memorize-context pointer are copied with guarded reads and logged as `*_status=copied|unreadable`; when the copied state already explains the observed return, the same line now also emits `inferred_exit_reason=...`. The follow-up gate still logs `SpellUsabilityTrace target=PostCanStartMemmingFollowupGate ... original_result=<n>` when that downstream helper is reached, and validation for that hook accepts the bounded proven two-hop thunk chain `0x8db151 -> 0x8db146 -> 0x8dc0f5`, following the second hop by hotpatch-thunk shape rather than exact byte identity.
...
To widen packet metadata logging from the default sampled policy to every observed send/receive for one short debugging session, set this additional variable before launch:

```powershell
$env:MONOMYTH_ENABLE_FULL_PACKET_TRACE = "1"
```
...
`MONOMYTH_ENABLE_FULL_PACKET_TRACE=1` is also optional and separate. Without it, packet metadata stays on the normal bounded policy (`first_50_then_every_500`). With it, the existing packet observer logs every observed send/receive metadata line for that process lifetime, which is useful for short bounded sessions such as login -> scribe -> memorize -> logout. It does not widen payload introspection, does not alter packet flow, and should be treated as a temporary local debugging mode because the log volume grows quickly.
