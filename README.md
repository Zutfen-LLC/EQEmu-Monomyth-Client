...
- `MemorizeSendPacketWrapper` resolves from cleanroom RVA `0x004c41f0`, requires that same checked-in ROF2 SHA-256, validates its exact entry bytes, and validates that the `StartSpellScribe`-like callsite at `0x0035e6fb` still targets the wrapper.
